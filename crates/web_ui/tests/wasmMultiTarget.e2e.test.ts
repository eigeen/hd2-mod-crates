...
(runE2e ? test : test.skip)(
  ...
  async () => {
    ...
    const options = migrationOptions(variantCount);
    let outputCount = 0;
    ...
    const result = await migrate_equipment_variants(
      ...
      nativeDataSource(gameDataDir),
      {
        ...
        onFile: (outputPath: string, bytes: Uint8Array) => {
          outputCount += 1;
          maxOutputBytes = Math.max(maxOutputBytes, bytes.byteLength);
          totalOutputBytes += bytes.byteLength;
          zip?.add(outputPath, bytes);
        },
      },
    ) as { summary: { migratedCount: number } };

    expect(result.summary.migratedCount).toBe(variantCount);
    expect(outputCount).toBe(variantCount * 3);
    ...
    console.info({ maxOutputBytes, totalOutputBytes, variantCount });
  },
  300_000,
);
...
function migrationOptions(variantCount: number): UnifiedMigrateOptions {
  ...
  return {
    variants: targets.map((target) => ({
      mappings: [{ category: "Armor", sourceHash, targetHash: target.hash }],
    })),
